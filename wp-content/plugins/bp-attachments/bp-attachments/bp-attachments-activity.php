...
function bp_attachments_activity_attach_media( $args = array() ) {
	if ( 'nouveau' === bp_get_theme_compat_id() && isset( $_POST['_bp_attachments_medium_url'] ) ) { // phpcs:ignore
		...
		error_log( print_r( $medium_pathinfo, true ) );

		if ( ! is_array( $medium_pathinfo ) || ! isset( $medium_pathinfo['id'], $medium_pathinfo['path'] ) ) {
			return $args; // Invalid medium path info, exit early
		}
