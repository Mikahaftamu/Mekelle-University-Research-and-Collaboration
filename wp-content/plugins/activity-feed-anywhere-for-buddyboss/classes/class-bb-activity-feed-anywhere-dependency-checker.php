 ...
 */
class BB_Activity_Feed_Anywhere_Dependency_Checker {
	...
	public static function check_dependencies() {
		if ( ! self::check_php() ) {
			add_action( 'admin_notices', array( 'BB_Activity_Feed_Anywhere_Dependency_Checker', 'add_php_notice' ) );
		...
		if ( ! self::check_buddypress() ) {
			add_action( 'admin_notices', array( 'BB_Activity_Feed_Anywhere_Dependency_Checker', 'add_buddypress_notice' ) );
			add_action( 'admin_init', array( __CLASS__, 'deactivate_self' ) );
		...
		/*
		*
		* Currently not used because BuddyBoss is supported.
		*
		if ( ! self::check_buddyboss() ) {
			add_action( 'admin_notices', array( 'BB_Activity_Feed_Anywhere_Dependency_Checker', 'add_buddyboss_notice' ) );
			add_action( 'admin_init', array( __CLASS__, 'deactivate_self' ) );
	...
	public static function add_php_notice() {
		...
		$message = sprintf( __( '<strong>Activity Feed Anywhere For BuddyBoss</strong> requires a minimum PHP version of %1$s, but you are running %2$s.', 'activity-feed-anywhere-for-buddyboss' ), self::MINIMUM_PHP_VERSION, phpversion() );
		...
		printf(
			...
			esc_html__( 'Learn more about updating PHP', 'activity-feed-anywhere-for-buddyboss' ),
			/* translators: accessibility text */
			esc_html__( '(opens in a new tab)', 'activity-feed-anywhere-for-buddyboss' )
		);
		echo '</p></div>';
	}
	...
	/**
	 * Adds notice in WP Admin if BuddyBoss is not supported. Currently not used.
	 *
	 * @access private
	 */
	public static function add_buddyboss_notice() {
		...
		$message = __( '<strong>Activity Feed Anywhere For BuddyBoss</strong> is currently incompatible with BuddyBoss.', 'activity-feed-anywhere-for-buddyboss' );
		...
		printf(
			'<p><a class="button button-primary" href="%1$s" target="_blank" rel="noopener noreferrer">%2$s <span class="screen-reader-text">%3$s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
			esc_url( $spaces_url ),
			esc_html__( 'Learn about BuddyPress', 'activity-feed-anywhere-for-buddyboss' ),
			/* translators: accessibility text */
			esc_html__( '(opens in a new tab)', 'activity-feed-anywhere-for-buddyboss' )
		);
		echo '</p></div>';
	...
	public static function add_buddypress_notice() {
		...
		$message = __( '<strong>Activity Feed Anywhere For BuddyBoss</strong> requires BuddyPress to be installed and activated.', 'activity-feed-anywhere-for-buddyboss' );
	...
	public static function deactivate_self() {
		deactivate_plugins( BB_AFA_PLUGIN_BASENAME );
	}
